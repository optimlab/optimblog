<?php
/**
 * @package    OptimBlog
 * @version    3.1.0.1
 * @author     Dmitriy Khokhlov <admin@optimlab.com>
 * @copyright  Copyright (c) 2018, Dmitriy Khokhlov. (https://optimlab.com/)
 * @license    https://opensource.org/licenses/GPL-3.0
 * @link       https://optimcart.com
 */
// Heading
$_['heading_title']                                     = 'OptimBlog';

// Text
$_['text_extension']                                    = 'Расширения';
$_['text_success']                                      = 'Настройки OptimBlog успешно изменены!';
$_['text_edit']                                         = 'Редактирование';
$_['text_general']                                      = 'Основное';
$_['text_exclusion']                                    = 'Полное исключение';
$_['text_information']                                  = 'Статьи';
$_['text_author']                                       = 'Автор';
$_['text_date']                                         = 'Дата';
$_['text_manufacturer']                                 = 'Производитель';
$_['text_review']                                       = 'Отзывы';
$_['text_view']                                         = 'Вид отображения';
$_['text_sort']                                         = 'Сортировка';
$_['text_limit']                                        = 'Лимит';
$_['text_image']                                        = 'Изображения';
$_['text_default']                                      = 'По умолчанию';
$_['text_date_asc']                                     = 'Дате (начиная с давней)';
$_['text_date_desc']                                    = 'Дате (начиная с последней)';
$_['text_name_asc']                                     = 'Название (А - Я)';
$_['text_name_desc']                                    = 'Название (Я - А)';
$_['text_rating_asc']                                   = 'Рейтинг (начиная с низкого)';
$_['text_rating_desc']                                  = 'Рейтинг (начиная с высокого)';
$_['text_share']                                        = 'Поделиться';
$_['text_seo']                                          = 'SEO';
$_['text_backup']                                       = 'Резервное Копирование и Восстановление';
$_['text_import']                                       = 'Файл успешно загружен!';
$_['text_list']                                         = 'Список';
$_['text_version']                                      = 'Успешно: Вы используете последнюю версию <strong>%s</strong>';
$_['text_version_info']                                 = 'Ваша текущая версия OptimBlog устарела! Текущая последняя версия <strong>%s</strong>';
$_['text_module']                                       = 'Расширения для OptimBlog';
$_['text_modules']                                      = 'Модули';
$_['text_modifications']                                = 'Модификаторы';
$_['text_download']                                     = 'Загрузка';
$_['text_install']                                      = 'Установка';
$_['text_loading']                                      = 'Загрузка..';
$_['text_adaptation']                                   = 'Адаптация';

// Entry
$_['entry_status']                                      = 'Статус';
$_['entry_information']                                 = 'Статьи';
$_['entry_information_limit']                           = 'Лимит статей (Витрина)';
$_['entry_information_description_length']              = 'Количество символов (Витрина)';
$_['entry_information_count']                           = 'Количество статей в категории';
$_['entry_information_author']                          = 'Отображение автора';
$_['entry_information_date']                            = 'Отображение даты';
$_['entry_information_review']                          = 'Отображение отзывов';
$_['entry_information_manufacturer']                    = 'Отображение производителя';
$_['entry_information_show']                            = 'Отображение статей в категориях';
$_['entry_category_author']                             = 'Отображение автора';
$_['entry_category_date']                               = 'Отображение даты';
$_['entry_category_review']                             = 'Отображение отзывов';
$_['entry_category_view']                               = 'Вид по умолчанию';
$_['entry_category_sort']                               = 'Сортировка';
$_['entry_category_sort_show']                          = 'Отображение сортировки';
$_['entry_category_limit_show']                         = 'Отображение лимита';
$_['entry_category_view_show']                          = 'Отображение кнопок';
$_['entry_image_category']                              = 'Размер изображения в списке категорий';
$_['entry_image_thumb']                                 = 'Размер большого изображения статьи';
$_['entry_image_popup']                                 = 'Размер всплывающего изображения статьи';
$_['entry_image_information']                           = 'Размер изображения в списке статей';
$_['entry_image_additional']                            = 'Размер дополнительных изображений статьи';
$_['entry_image_related']                               = 'Размер изображения аналогичных статей';
$_['entry_image_category_popup']                        = 'Размер дополнительных всплывающих изображений категории';
$_['entry_image_category_additional']                   = 'Размер дополнительных изображений категории';
$_['entry_width']                                       = 'Ширина';
$_['entry_height']                                      = 'Высота';
$_['entry_review']                                      = 'Разрешить отзывы';
$_['entry_review_guest']                                = 'Гостевой отзыв';
$_['entry_captcha']                                     = 'Защита от роботов';
$_['entry_category']                                    = 'Категории';
$_['entry_exclusion_information']                       = 'Статьи';
$_['entry_exclusion_information_author']                = 'Исключение';
$_['entry_exclusion_information_date']                  = 'Исключение';
$_['entry_exclusion_information_review']                = 'Исключение';
$_['entry_exclusion_information_manufacturer']          = 'Исключение';
$_['entry_exclusion_category_author_information']       = 'Исключение для статей в категориях';
$_['entry_exclusion_category_date_information']         = 'Исключение для статей в категориях';
$_['entry_exclusion_category_review_information']       = 'Исключение для статей в категориях';
$_['entry_exclusion_category_manufacturer_information'] = 'Исключение для статей в категориях';
$_['entry_exclusion_category_author']                   = 'Исключение';
$_['entry_exclusion_category_date']                     = 'Исключение';
$_['entry_exclusion_category_review']                   = 'Исключение';
$_['entry_share']                                       = 'Введите Код';
$_['entry_thumb']                                       = 'Отображение главного изображения';
$_['entry_link']                                        = 'Путь к файлу';
$_['entry_style']                                       = 'Стили для дополнительных изображений';
$_['entry_script']                                      = 'Скрипты для дополнительных изображений';
$_['entry_canonical_category_product']                  = 'Канонический URL категории товаров';
$_['entry_canonical_category_information']              = 'Канонический URL категории статей';
$_['entry_canonical_information']                       = 'Канонический URL статьи';
$_['entry_breadcrumbs_category_product']                = 'Хлебные крошки категории товаров';
$_['entry_breadcrumbs_category_information']            = 'Хлебные крошки категории статей';
$_['entry_breadcrumbs_product']                         = 'Хлебные крошки товара';
$_['entry_breadcrumbs_information']                     = 'Хлебные крошки статьи';
$_['entry_backup']                                      = 'Резервное копирование';
$_['entry_restore']                                     = 'Восстановление';
$_['entry_adaptation']                                  = 'Модификатор';

// Tab
$_['tab_article']                                       = 'Статьи';
$_['tab_category']                                      = 'Категории';

// Column
$_['column_name']                                       = 'Название магазина';
$_['column_status']	                                    = 'Статус';
$_['column_action']                                     = 'Действие';

// Help
$_['help_category_view']                                = 'Определяет, форматирование отображением статей на странице категории.';
$_['help_category_sort']                                = 'Определяет, последовательность отображения статей на странице категории.';
$_['help_information_limit']                            = 'Определяет, сколько статей отображается на странице категории.';
$_['help_information_description_length']               = 'Количество символов в кратком описании статьи в категории.';
$_['help_review']                                       = 'Включить/Отключить отзывы.';
$_['help_review_guest']                                 = 'Разрешить гостям оставлять отзывы.';
$_['help_exclusion_information']                        = '(Автозаполнение)';
$_['help_exclusion_category']                           = '(Автозаполнение)';
$_['help_information_count']                            = 'Показывает количество статей в подкатегориях в пользовательской части в верхнем меню. Осторожно!!! Данная настройка увеличивает нагрузку на сервер при большом количестве подкатегорий!';
$_['help_captcha']                                      = 'Включение защиты от роботов.';
$_['help_canonical_category']                           = 'Материнские категории в канонический URL подкатегории. Например: /category/subcategory/subcategory';
$_['help_canonical_information']                        = 'Подкатегории в канонический URL статьи, при наличии главной категории в её настройках. Например: /category/subcategory/article';
$_['help_breadcrumbs_category']                         = 'Полный путь при коротком URL /subcategory. Например: /category/subcategory/subcategory';
$_['help_breadcrumbs_product']                          = 'Полный путь при коротком URL /product. Например: /category/subcategory/product. При наличии главной категории в настройках.';
$_['help_breadcrumbs_information']                      = 'Полный путь при коротком URL /article. Например: /category/subcategory/article. При наличии главной категории в настройках.';
$_['help_backup']                                       = 'Резервное копирование настроек из Базы Данных';
$_['help_restore']                                      = 'Восстановление настроек в Базу Данных';
$_['help_theme']                                        = 'Скачайте и установите сгенерированный модификатор. Если модификатор не помогает, то обратитесь за помощью к вашему разработчику чтоб отредактировать этот файл.';

// Buttons
$_['button_block']                                      = 'На всю ширину';
$_['button_list']                                       = 'Список';
$_['button_grid']                                       = 'Сетка';
$_['button_position']                                   = 'Позиция';
$_['button_extension']                                  = 'Дополнительные модули';
$_['button_upgrade']                                    = 'Обновление';
$_['button_installed']                                  = 'Установлено';
$_['button_adaptation']                                 = 'Сгенерировать';

// Error
$_['error_warning']                                     = 'Ошибка! Проверьте форму на наличие ошибок!';
$_['error_permission']                                  = 'У Вас нет прав для управления данным модулем!';
$_['error_file']                                        = 'Файл не найден!';
$_['error_filetype']                                    = 'Неправильный тип файла!';
$_['error_datatype']                                    = 'Неправильный тип данных в файле!';
$_['error_limit']                                       = 'Укажите лимит!';
$_['error_image_thumb']                                 = 'Необходимо указать размер большого изображения статьи!';
$_['error_image_popup']                                 = 'Необходимо указать размер всплывающего изображения статьи!';
$_['error_image_information']                           = 'Необходимо указать размер изображения в списке статей!';
$_['error_image_category']                              = 'Необходимо указать размер изображения в списке категорий!';
$_['error_image_additional']                            = 'Необходимо указать размер дополнительных изображений статьи!';
$_['error_image_related']                               = 'Необходимо указать размер изображения аналогичных статей!';
$_['error_image_category_popup']                        = 'Необходимо указать размер дополнительных всплывающих изображений категории!';
$_['error_image_category_additional']                   = 'Необходимо указать размер дополнительных изображений категории!';
$_['error_install']                                     = 'Выполняется установка расширения, пожалуйста, подождите несколько секунд, прежде чем попытаться установить заново!';
$_['error_download']                                    = 'Расширение не может быть загружено!';
$_['error_unzip']                                       = 'Zip архив не удалось открыть!';
$_['error_connection']                                  = 'Не удалось подключиться к GitHub';

// Modules
$_['optimblog_latest']                                  = 'Новые статьи';
$_['optimblog_featured']                                = 'Рекомендуемые статьи';
$_['optimblog_best']                                    = 'Лучшие статьи';
$_['optimblog_category']                                = 'Категории Статей';
$_['optimblog_search']                                  = 'Поиск статей';
$_['optimblog_latest_description']                      = 'Можно использовать как модуль вывода Новостей или Новых статей.';
$_['optimblog_featured_description']                    = 'Рекомендуемые статьи';
$_['optimblog_best_description']                        = 'Сортировка по Рейтингу или Просмотрам.';
$_['optimblog_category_description']                    = 'Модуль выводит Категории статей, аналогично стандартному модулю Opencart «Категории».';
$_['optimblog_search_description']                      = 'Модуль выводит форму строки Поиска статей, аналогично стандартному модулю Opencart «Поиск».';

// Modifications
$_['optimblog-admin-filter']                            = 'Admin Filter';
$_['optimblog-admin-filter_description']                = '<p>Расширяет фильтрацию Категорий, Товаров и Статей:</p><ol><li>Тип категории: Товары или Статьи</li><li>Принадлежность к категории или её отсутствия.</li><li>Принадлежность к производителю или его отсутствия.</li><li>Кол-во просмотров статей.</li></ol>';

// URL
$_['optimcart_url']                                     = 'https://optimcart.ru/';